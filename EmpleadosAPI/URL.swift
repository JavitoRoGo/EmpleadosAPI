...
// Un endpoint es como una función remota, que está y se ejecuta en otro sitio, en la API; y se invoca a través de urls
// Por tanto, lo primero que necesitamos son las urls para hacer get, post, put, etc.
...
// Estos diferentes entornos se suelen hacer con ficheros de configuración de Xcode o .plist, pero es preferible hacerlo de otra forma. Porque esto podría implicar que puedas subir a GitHub datos como contraseñas de acceso a bases de datos. Al menos hay que poner estos archivos dentro del .gitignore
// O sea, no es buena idea generar un plist para poner las urls de conexión. La forma de Julio de crear diferentes formas de trabajo es poner una variable global con la url raíz, y hacer extensiones para cada tipo de petición o url
...
// Por ejemplo, con estos constructores, al usar .appending nos añade el caracter / para encadenar las partes de la url
...
extension URL {
	...
	static func searchEmpleado(search: String) -> URL {
		...
		// esto devolvería la url base más la búsqueda: api/searchEmpleado?=valor_a_buscar
	}
}
